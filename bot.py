...
def telegram_api(method, data=None, timeout=20):
    ...
    try:
        ...
        if not result.get("ok"):
            logging.error("Telegram error: %s", result)
...
def send_telegram(message, chat_id=None):
    ...
    if not target:
        logging.error("No Telegram chat ID")
        return False
...
def telegram_startup_test():
    if not config_ok():
        logging.error(
            "Missing TELEGRAM_TOKEN, TELEGRAM_CHAT_ID or TWELVE_DATA_KEY"
        )
        return
    ...
    if result and result.get("ok"):
        ...
        send_telegram(
            ...
            "Send /signal to request a manual XAU/USD analysis."
        )
    else:
...
def strong_body(c):
    return body(c) / candle_range(c) >= 0.60
...
def bullish_rejection(c):
    return (
        lower_wick(c) >= body(c) * 1.5
        and lower_wick(c) > upper_wick(c)
    )


def bearish_rejection(c):
    return (
        upper_wick(c) >= body(c) * 1.5
        and upper_wick(c) > lower_wick(c)
    )
...
def displacement(c):
    return (
        strong_body(c)
        and body(c) / candle_range(c) >= 0.70
    )
...
def structure_analysis(candles):
    ...
    highs = [x["high"] for x in recent[:-2]]
    lows = [x["low"] for x in recent[:-2]]
...
def liquidity_sweep(candles):
    ...
    high = max(x["high"] for x in previous)
    low = min(x["low"] for x in previous)

    # Sell-side liquidity sweep then bullish rejection
    if (
        last["low"] < low
        and last["close"] > low
        and bullish_rejection(last)
    ):
        return 20, "BULLISH LIQUIDITY SWEEP"

    # Buy-side liquidity sweep then bearish rejection
    if (
        last["high"] > high
        and last["close"] < high
        and bearish_rejection(last)
    ):
        return 20, "BEARISH LIQUIDITY SWEEP"
...
def analyze_market():
    m1 = get_candles("1min", 80)
    m5 = get_candles("5min", 80)
    m15 = get_candles("15min", 80)
    ...
    # M15 trend
    m15_trend_score, m15_trend = trend_score(m15)

    # M5 trend / structure
    m5_trend_score, m5_trend = trend_score(m5)
    structure_score, structure = structure_analysis(m5)

    # Liquidity
    liquidity_score, liquidity = liquidity_sweep(m5)

    # M1 entry
    entry_score, entry_signal = entry_analysis(m1)
    ...
    # Unique setup based on closed M5 candle
    setup_source = (
        m5[-1]["time"]
        + direction
        + str(round(entry, 2))
...
def format_signal(a, manual=False):
    ...
    return (
        f"{emoji} <b>XAU/USD {a['direction']} — {mode}</b>\n\n"
...
def scanner():
    ...
    logging.info("Automatic XAU/USD scanner started.")
...
def handle_command(text, chat_id):
    ...
    if command.startswith("/start"):
        ...
        send_telegram(
            "🟢 <b>ritamariagold</b>\n\n"
            "XAU/USD candle-analysis bot is online.\n\n"
            "<b>Commands:</b>\n"
        ...
        if analysis.get("valid"):
            ...
            # Manual requests are always answered.
            # This prevents the old bot problem where
            # repeated /signal requests appeared dead.
